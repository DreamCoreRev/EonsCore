...
void BattlegroundTPScore::BuildObjectivesBlock(WorldPackets::Battleground::PVPLogData_Player& playerData)
{
...
void BattlegroundTP::PostUpdateImpl(uint32 diff)
{
    ...
    {
        // Nouvelle verification pour les 27 minutes
        if (GetStartTime() >= 27 * MINUTE * IN_MILLISECONDS)
        {
            if (GetTeamScore(TEAM_ALLIANCE) == 0)
            {
                if (GetTeamScore(TEAM_HORDE) == 0)        // Aucun score - match nul
                    EndBattleground(0);
                else                                      // La Horde a plus de points et gagne
            ...
            {
                // La Horde a plus de points et gagne
                EndBattleground(HORDE);
            }
            else
            {
...
void BattlegroundTP::AddPlayer(Player* player)
{
    ...
    if (!isInBattleground)
        PlayerScores[player->GetGUID()] = new BattlegroundTPScore(player->GetGUID());
}
...
WorldSafeLocsEntry const* BattlegroundTP::GetClosestGraveyard(Player* player)
{
    ...
    if (isRated())
    {
        if ((player->GetAreaId() == 5005 && team == TEAM_HORDE) || (player->GetAreaId() == 5005 && team == TEAM_ALLIANCE))
...
void BattlegroundTP::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
        ...
        && BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        ...
        player->StartTimedAchievement(AchievementCriteriaTimedTypes::ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET, BG_TP_SPELL_ALLIANCE_FLAG_PICKED);
        ...
        && BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
        ...
        player->StartTimedAchievement(AchievementCriteriaTimedTypes::ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET, BG_TP_SPELL_HORDE_FLAG_PICKED);
    ...
    player->RemoveAurasWithInterruptFlags(SpellAuraInterruptFlags::AURA_INTERRUPT_FLAG_ENTER_PVP_COMBAT);
}
