...
void BattlegroundBFGScore::BuildObjectivesBlock(WorldPackets::Battleground::PVPLogData_Player& playerData)
{
...
BattlegroundBFG::BattlegroundBFG()
{
    ...
    StartMessageIds[BG_STARTING_EVENT_FIRST] = BG_BFG_TEXT_START_TWO_MINUTES;
    StartMessageIds[BG_STARTING_EVENT_SECOND] = BG_BFG_TEXT_START_ONE_MINUTE;
    StartMessageIds[BG_STARTING_EVENT_THIRD] = BG_BFG_TEXT_START_HALF_MINUTE;
    StartMessageIds[BG_STARTING_EVENT_FOURTH] = BG_BFG_TEXT_BATTLE_HAS_BEGUN;
...
void BattlegroundBFG::AddPlayer(Player* player)
{
    ...
    if (!isInBattleground)
        PlayerScores[player->GetGUID()] = new BattlegroundBFGScore(player->GetGUID());
}
...
void BattlegroundBFG::EventPlayerClickedOnFlag(Player* source, GameObject* /*target_obj*/)
{
    ...
    source->RemoveAurasWithInterruptFlags(SpellAuraInterruptFlags::AURA_INTERRUPT_FLAG_ENTER_PVP_COMBAT);
    uint32 sound = 0;
