...
// - 88888
class spell_vehicle_ride_toc : public SpellScriptLoader
{
public:
    spell_vehicle_ride_toc() : SpellScriptLoader("spell_vehicle_ride_toc") {}

    class spell_vehicle_ride_toc_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_vehicle_ride_toc_SpellScript);
        ...
        SpellCastResult CheckCast()
        {
            ...
            return SPELL_FAILED_CUSTOM_ERROR; // Prevent the cast
        }

        void Register() override
        {
            OnCheckCast += SpellCheckCastFn(spell_vehicle_ride_toc_SpellScript::CheckCast);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_vehicle_ride_toc_SpellScript();
    }
};
...
public:
    ...
    struct boss_grand_championAI : public EscortAI
    {
        ...
        void WaypointReached(uint32 i, uint32) override
        {
            if (!pInstance)
                return;
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
            ...
            {
                if (id == 5)
                    me->SetFacingTo(4.7123889f);
                else if (id == 7) // reached new mount!
                {
                    if (!NewMountGUID.IsEmpty())
                            ...
                            for( Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr )
                                if (Player* plr = itr->GetSource())
                                    if (me->GetExactDist(plr) <= 5.0f )
                                        if (Vehicle* v = plr->GetVehicle())
                                            if (Unit* c = v->GetBase())
                                                if (c->IsCreature() && c->ToCreature()->GetEntry() == static_cast<uint32>(pInstance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE ? VEHICLE_ARGENT_BATTLEWORG : VEHICLE_ARGENT_WARHORSE))
                                                {
                                                    me->GetMotionMaster()->Clear();
