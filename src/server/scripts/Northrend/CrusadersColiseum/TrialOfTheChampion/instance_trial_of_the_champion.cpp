...
#define CLEANUP_CHECK_INTERVAL  5000ms
...
public:
    ...
    {
        instance_trial_of_the_champion_InstanceMapScript(InstanceMap* pMap) : InstanceScript(pMap)
        {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            // Grand Champions Mount:
            case MOKRA_MOUNT:
                if (teamInInstance == TEAM_HORDE)
                    creature->UpdateEntry(JACOB_MOUNT);
                break;
            case ERESSEA_MOUNT:
                if (teamInInstance == TEAM_HORDE)
                    creature->UpdateEntry(AMBROSE_MOUNT);
                break;
            case RUNOK_MOUNT:
                if (teamInInstance == TEAM_HORDE)
                    creature->UpdateEntry(COLOSOS_MOUNT);
                break;
            case ZULTORE_MOUNT:
                if (teamInInstance == TEAM_HORDE)
                    creature->UpdateEntry(JAELYNE_MOUNT);
                break;
            case VISCERI_MOUNT:
                if (teamInInstance == TEAM_HORDE)
                    creature->UpdateEntry(LANA_MOUNT);
                break;
                ...
                if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                {
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* c = instance->GetCreature(NPC_GrandChampionGUID[i]))
                        {
                            ...
                            {
                            case 0:
                                if (Creature* pBoss = announcer->SummonCreature(entry, 736.695f, 650.02f, 412.4f, 4.7123889f))
                                {
                                ...
                                break;
                            case 1:
                                if (Creature* pBoss = announcer->SummonCreature(entry, 756.32f, 650.05f, 412.4f, 4.7123889f))
                                {
                                ...
                                break;
                            case 2:
                                if (Creature* pBoss = announcer->SummonCreature(entry, 746.5f, 650.65f, 411.7f, 4.7123889f))
                                {
            ...
            case DATA_GRAND_CHAMPION_DIED:
                if (++Counter >= 3)
                {
                    ...
                    if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                    {
                        ...
                        announcer->SummonGameObject(instance->IsHeroic() ? GO_CHAMPIONS_LOOT_H : GO_CHAMPIONS_LOOT, 746.59f, 618.49f, 411.09f, 1.42f, QuaternionData(), 25h); // [LOOT]
        ...
        void DoSummonGrandChampion(uint32 BossNumber, uint8 BossOrder)
        {
            ...
            if (Creature* pBoss = instance->SummonCreature(CHAMPION_TO_SUMMON, SpawnPos))
            {
                ...
                pBoss->ToCreature()->SetHomePosition(748.309f, 619.448f, 411.3f, 1.5707963f);
                ...
                for (uint8 i = 0; i < 3; ++i)
                    if (Creature* pAdd = instance->SummonCreature(MINION_TO_SUMMON, SpawnPos))
                    {
                        ...
                        pAdd->SetHomePosition(748.309f, 619.448f, 411.3f, 1.5707963f);
        ...
        void Update(uint32 diff) override
        {
            ...
            case EVENT_ARGENT_CHALLENGE_LEAVE_CHEST:
            {
                if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                    if (Creature* boss = instance->GetCreature(NPC_ArgentChampionGUID))
                    {
                        ...
                        announcer->SummonGameObject(chest, 746.59f, 618.49f, 411.09f, 1.42f, QuaternionData(), 25h); // [LOOT]
                    }
            ...
            case EVENT_SUMMON_BLACK_KNIGHT:
            {
                if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                    if (Creature* bk_vehicle = announcer->SummonCreature(VEHICLE_BLACK_KNIGHT, 769.834f, 651.915f, 447.035f, 0.0f))
                    {
                        ...
                        bk_vehicle->SetFacingTo(3.1415927f);
            ...
            case EVENT_START_BLACK_KNIGHT_SCENE:
            {
                ...
                {
                    Position exitPos = { 751.003357f, 638.145508f, 411.570129f, 3.1415927f };
            ...
            case EVENT_BLACK_KNIGHT_CAST_ANNOUNCER:
            {
                if (Creature* announcer = instance->GetCreature(NPC_AnnouncerGUID))
                {
                    if (Creature* bk = instance->GetCreature(NPC_BlackKnightGUID))
                    {
                        bk->NearTeleportTo(745.016f, 631.506f, 411.575f, bk->GetAbsoluteAngle(announcer->GetPositionX(), announcer->GetPositionY()));
            ...
            case EVENT_BLACK_KNIGHT_ATTACK:
            {
                if (Creature* bk = instance->GetCreature(NPC_BlackKnightGUID))
                {
                    ...
                    bk->ReplaceAllNpcFlags(UNIT_NPC_FLAG_NONE);
